...
## A standing hazard: the dispatch pipeline deletes this directory

The archival-dispatch publisher commits its whole working tree from a checkout it
made earlier. Anything added after that checkout is deleted by its next commit.
It has now destroyed work three times:

- `3c38cb8` deleted `posts/2026-09-02-register-awl.md`
- `748f42f` deleted **the entire `papers/` directory**, 1,126 files in a commit
  whose message says it publishes a dispatch

Nothing here is safe while that publisher runs from a stale checkout. The fix
belongs in the publisher: it should add its own files rather than commit a whole
tree. Until then, run `npm run papers:verify` after any batch lands, and check
that `papers/` still exists.
